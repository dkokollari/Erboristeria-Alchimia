...
else
{
  ...
  $start = (($page > 1) ? $page * $numero_risultati - $numero_risultati : 0);

  // sezione memorizza scelta combobox
  switch ($ordina_utenti)
  {
    case "":
      $contenuto = str_replace('<option value="">&nbsp;</option>', '<option selected="selected" value="">&nbsp;</option>', $contenuto);
    break;
    case "timbri":
      $contenuto = str_replace('<option value="timbri">Timbri</option>', '<option selected="selected" value="timbri">Timbri</option>', $contenuto);
    break;
    case "tipo_utente":
      $contenuto = str_replace('<option value="tipo_utente">Tipo utente</option>', '<option selected="selected" value="tipo_utente">Tipo utente</option>', $contenuto);
    break;
    case "data_nascita_ASC":
      $contenuto = str_replace('<option value="data_nascita_ASC">Data nascita ASC</option>', '<option selected="selected" value="data_nascita_ASC">Data nascita ASC</option>', $contenuto);
    break;
    case "data_nascita_DESC":
      $contenuto = str_replace('<option value="data_nascita_DESC">Data nascita DESC</option>', '<option selected="selected" value="data_nascita_DESC">Data nascita DESC</option>', $contenuto);
    break;
    case "data_registrazione_ASC":
      $contenuto = str_replace('<option value="data_registrazione_ASC">Data registrazione ASC</option>', '<option selected="selected" value="data_registrazione_ASC">Data registrazione ASC</option>', $contenuto);
    break;
    case "data_registrazione_DESC":
      $contenuto = str_replace('<option value="data_registrazione_DESC">Data registrazione DESC</option>', '<option selected="selected" value="data_registrazione_DESC">Data registrazione DESC</option>', $contenuto);
    break;
  };
  switch ($numero_risultati)
  {
    case "20":
      $contenuto = str_replace('<option value="20">20</option>', '<option selected="selected" value="20">20</option>', $contenuto);
    break;
    case "40":
      $contenuto = str_replace('<option value="40">40</option>', '<option selected="selected" value="40">40</option>', $contenuto);
    break;
    case "80":
      $contenuto = str_replace('<option value="80">80</option>', '<option selected="selected" value="80">80</option>', $contenuto);
    break;
  };
  ...
  else
  {
    ...
    if ($page > 1)
    {
      $pages_menu = '<a href="admin.php?page=' . ($page - 1) . "&amp;email_search=$email_search&amp;ordina_utenti=$ordina_utenti" . "&amp;numero_risultati=$numero_risultati&amp;search=Invia" . '" class="classic_btn always_visible">Indietro</a>' . "\n";
    }
    for ($n_page = 1;$n_page <= $total_pages;$n_page++)
    {
      if ($n_page != $page && ($n_page > ($page + 1) || $n_page < ($page - 1)))
      {
        $pages_menu .= '<a href="admin.php?page=' . $n_page . "&amp;email_search=$email_search&amp;ordina_utenti=$ordina_utenti" . "&amp;numero_risultati=$numero_risultati&amp;search=Invia" . '" class="classic_btn hidden">' . $n_page . '</a>' . "\n";
      }
      else if ($n_page != $page)
      {
        $pages_menu .= '<a href="admin.php?page=' . $n_page . "&amp;email_search=$email_search&amp;ordina_utenti=$ordina_utenti" . "&amp;numero_risultati=$numero_risultati&amp;search=Invia" . '" class="classic_btn">' . $n_page . '</a>' . "\n";
      }
      else
      {
    ...
    if ($page < $total_pages)
    {
      $pages_menu .= '<a href="admin.php?page=' . ($page + 1) . "&amp;email_search=$email_search&amp;ordina_utenti=$ordina_utenti" . "&amp;numero_risultati=$numero_risultati&amp;search=Invia" . '" class="classic_btn always_visible">Avanti</a>' . "\n";
    }
