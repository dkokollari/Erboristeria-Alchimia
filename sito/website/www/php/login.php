...
{ // utente con cookie senza sessione
  $aux = new DBAccess();
  ...
  else
  {
    ...
    if (empty($temp) || $_COOKIE['password'] != $temp[0]['password_utente'])
    { // utente non trovato o password diversa
...
if ($_POST['Login'])
{
  ...
  else if (!filter_var($email, FILTER_VALIDATE_EMAIL) || (strlen($password) < $minLengthPwd || strlen($password) > $maxLengthPwd))
  {
    $errore = $errore_wrong;
  }
  else
...
else if ($_POST['Registrati'])
{ // redirect dalla pagina registrazione
...
{ // guardare DBAccess::getSingolo_Utenti() per vedere la struttura di $array
  $_SESSION['auth'] = true;
